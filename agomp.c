...
void test(char *BUFFOR) {
    ...
    int k2 = n;
    {
        ...
        for (L = n; L >= 2; L--) {
            ...
            i--;
            if (s == 0) {
                e[L] = h; e2[L] = h*h; a[j] = 0.0;
            } else {
                s += h*h; e2[L] = s; g = sqrtf(s);
                if (h >= 0.0) g = -g;
                e[L] = g;
                s = 1.0 / (s - h*g);
    ...
    #pragma omp parallel
    {
        #pragma omp single
        {
            ...
            while (fgets(buffer, BUFSIZE - 1, stdin)) {
                ...
                batch[idx++] = strdup(buffer);

                if (idx == BATCH_SIZE) {
                    #pragma omp task firstprivate(batch, idx)
                    {
...
// #define BUFSIZE 1024
// #define NMAX 20
// #define BATCH_SIZE 128       // liczba grafów w jednej paczce

// /* test() – identyczny jak w AGS, zakończony sekcją krytyczną */
// void test(char *BUFFOR) {
//     int i, j, k, k3, k4, L, L1, z;
//     double eps, g, h, ma, mn, norm, s, t, u, w;
//     int cond;
...
//                 batch[batch_idx++] = strdup(buffer);

//                 if (batch_idx == BATCH_SIZE) {
//                     // Paczka pełna – utwórz zadanie, które ją przetworzy
//                     #pragma omp task firstprivate(batch, batch_idx) \
//                         private(buffer) // buffer jest już skopiowany?
//                     {
//                         for (int t = 0; t < BATCH_SIZE; t++) {
//                             test(batch[t]);
//                             free(batch[t]);
//                         }
//                     }
//                     batch_idx = 0;  // nowa paczka
//                 }
//             } // while

//             // Pozostałe wiersze (niepełna paczka)
//             if (batch_idx > 0) {
//                 #pragma omp task firstprivate(batch, batch_idx)
//                 {
...
// /* test() – identyczny jak w AGS, zakończony printf (patrz krytyczna sekcja) */
// void test(char *BUFFOR) {
//     int i, j, k, k3, k4, L, L1, z;
//     double eps, g, h, ma, mn, norm, s, t, u, w;
//     int cond;
...
//         }
//     }
//     /* Wypisanie wyniku chronione sekcją krytyczną */
//     #pragma omp critical
//     {
//         printf("%s\n", BUFFOR);
//     }
...
//                 // każdy graf przetwarzamy w osobnym zadaniu OpenMP
...
//                 }
//             }
//         } // koniec single – tu wątki są synchronizowane
//         #pragma omp taskwait
//     } // koniec parallel – wszystkie zadania zakończone
